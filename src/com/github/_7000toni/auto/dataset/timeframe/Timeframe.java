	...
	private int size(boolean replayMode) {
		...
		if (tickBased) {
			...
			if (replayMode) {
				...
				while(true) {					
					if (size == 1 && !add || size == data.size() && add || data.get(size).dateTime().isEqual(currDate) || data.get(size).dateTime().isBefore(currDate) && !add || data.get(size).dateTime().isAfter(currDate) && add) {
						break;
					} else {
						size += add?1:-1;
					}
